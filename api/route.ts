...
async function startBot() {
    ...
    // MENGGUNAKAN chatUpdate AGAR BISA MEMFILTER HISTORY LAMA
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        // HANYA MERESPON PESAN BARU (Bukan Sinkronisasi Masa Lalu)
        ...
        try {
            ...
            // LOGIKA CONSOLE UNTUK MELIHAT PESAN MASUK
            if (command) {
                console.log(`[PESAN MASUK] Command: ${command} dari ${senderNumber}`);
            }
...
*IG: @elthoriqqqq_*`;
            ...
            console.log("Error di sistem pesan:", err.message); 
        }
    });

    sock.ev.on('connection.update', async (u) => {
        if (u.qr) qrcode.generate(u.qr, { small: true });
        if (u.connection === 'open') console.log('✅ BOT PEDIABOY JALAN (MENU NORMAL + BUG FIX)');
